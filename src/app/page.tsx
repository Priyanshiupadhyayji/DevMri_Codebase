        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
          ...
          {session?.user ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 14px', background: 'rgba(0,230,118,0.1)', border: '1px solid rgba(0,230,118,0.25)', borderRadius: 100 }}>
                {session.user.image && <img src={session.user.image} alt="" style={{ width: 22, height: 22, borderRadius: '50%', border: '1.5px solid var(--health-green)' }} />}
                <span style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--health-green)', fontFamily: 'var(--font-mono)' }}>{session.user.name?.split(' ')[0] || 'User'}</span>
              </div>
              <button onClick={() => signOut()} style={{ padding: '6px 12px', background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--text-muted)', fontSize: '0.7rem', fontWeight: 700, borderRadius: 6, cursor: 'pointer' }}>Sign Out</button>
            </div>
          ) : (
            <button onClick={() => signIn('github')} style={{ padding: '8px 20px', background: 'transparent', border: '1.5px solid rgba(0,229,255,0.3)', color: 'var(--scan-cyan)', fontWeight: 800, borderRadius: 8, cursor: 'pointer', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: 8, transition: 'all 0.3s ease' }}>
